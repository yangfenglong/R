args <- commandArgs(T)
if(length(args)<2){
    cat("usage: <pcoa.csv> <04.Diversity/group.list>  <pc1> <pc2> <output:pca.csv.summary.txt> <pca.pdf> <pca.png> \n")
    cat ("Example:  /PUBLIC/software/public/System/R-2.15.3/bin/Rscript plot.R pcoa.csv ../../group.list 14.67 12.95 pca.csv.summary.txt pca.pdf pca.png\n")
    quit("no")
}

pcoa.csv=args[1]
group.list=args[2]
data = read.table(file=pcoa.csv, head=T, na.strings=T,row.names=1,sep=",")
...
colnames(df.summary)<- c("group","pcoa1.mean","pcoa1.sd","pcoa1.lower","pcoa1.upper","pcoa2.mean","pcoa2.sd","pcoa2.lower","pcoa2.upper")
